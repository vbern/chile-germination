...
df <- read.csv(paste(out.dir, "/alldata_mod_2016-10-25.csv", sep = ""), header = T) ##rep 1 and 2, run 5
str(df)
...
survreg_run <- survreg(Surv(end, status) ~ factor(trt) + factor(region)  + trt:region + frailty(run) + frailty(plate%in%run), data = df1, dist = "exponential")
survreg_run2 <- survreg(Surv(end, status) ~ factor(trt) + factor(region) + trt:region+ frailty(run) + frailty(plate%in%run), data = df1, dist = "weibull")
survreg_run3 <- survreg(Surv(end, status) ~ factor(trt) + factor(region) + trt:region+ frailty(run) + frailty(plate%in%run), data = df1, dist = "lognormal")
survreg_run4 <- survreg(Surv(end, status) ~ factor(trt) + factor(region) + trt:region+ frailty(run) + frailty(plate%in%run), data = df1, dist = "loglogistic")
...
surv.reg <- survreg(Surv(end, status) ~ trt + I(trt^2) + region + trt:region #+ trt:line%in%region 
                    + frailty(run) + frailty(plate%in%run), 
