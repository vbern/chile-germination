...
plot(test.region.1, col = c("black", "red", "green", "cyan", "blue"), 
     conf.int = T, 
     lwd = 1.40, xlim = c(0,550), 
     #main = "Germination by region"
     )
legend(5, .2, c("Central Valleys", "E Coast", "Sierra Madre de la Sur", "W Coast", "Yucatan" ), 
       col =  c("black", "red", "green", "cyan", "blue"),lwd = 1.40, bty = "n", cex = 1)
title(xlab = "time (hours)", line = 2, ylab = "probability of not germinating")
...
cox.9 <- coxph(Surv(end,status) ~ ecoast + wcoast + trt20, data = df1)
cox.9 #all significant
cox.10 <- coxph(Surv(end,status) ~ ecoast + wcoast + trt20 + trt0, data = df1)
...
cox.14 <- coxph(Surv(end,status) ~ ecoast + wcoast + trt20 + trt0 + trt10 + yucatan + ecoast:trt20, data = df1)
cox.14 #NS
cox.15 <- coxph(Surv(end,status) ~ ecoast + wcoast + trt20 + trt0 + trt10 + yucatan + wcoast:trt20, data = df1)
cox.15 #NS
cox.16 <- coxph(Surv(end,status) ~ ecoast + wcoast + trt20 + trt0 +trt10 + yucatan + ecoast:trt0, data = df1)
cox.16 #NS
cox.17 <- coxph(Surv(end,status) ~ ecoast + wcoast + trt20 + trt0 + trt10 + yucatan +wcoast:trt0, data = df1)
cox.17 #NS
