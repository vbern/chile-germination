...
plot(test.20.1, col = c("black", "red", "grey", "green", "cyan", "blue"), lty = 4,
...
plot(test.15.1, col = c("black", "red", "grey", "green", "cyan", "blue"), lty = 3,
     lwd = 1.75, xlim = c(0,550), xaxt = "n"
)
axis(4)
axis(1, at=c(100,200,300,400,500), labels=c("","","","",""))
...
plot(test.10.1, col = c("black", "red", "grey", "green", "cyan", "blue"), lty = 2, 
     lwd = 1.75, xlim = c(0,550), xaxt = "n"
)
axis(4)
axis(1, at=c(100,200,300,400,500), labels=c("","","","",""))
...
plot(test.0.1, col = c("black", "red", "grey", "green", "cyan", "blue"), lty = 1,
     lwd = 1.75, xlim = c(0,550), xlab = "time (hours)")
text(450,.9, "0% PEG", cex = 2)
axis(4)
title(line = 2, ylab = "probability of not germinating")
legend(5, .35, c("Central Valleys", "E Coast", "Johnny's", "Sierra Madre de la Sur", "W Coast", "Yucatan" ), 
       col =  c("black", "red", "grey", "green", "cyan", "blue"),lwd = 1.40, bty = "n", cex = 1)
...
survreg_run <- survreg(Surv(end, status) ~ trt + region + trt:region + trt:line%in%region + frailty(run) + frailty(plate%in%run), data = df1, dist = "weibull")
survreg_run2 <- survreg(Surv(end, status) ~ trt + region + trt:region + frailty(run), data = df1, dist = "loglogistic")
survreg_run <- survreg(Surv(end, status) ~ trt + region + trt:region + frailty(run), data = df1, dist = "weibull")
survreg_run2 <- survreg(Surv(end, status) ~ trt + region + trt:region + frailty(run), data = df1, dist = "loglogistic")
...
out <- capture.output(survreg_run)
cat("RegressionOutput", out, file = paste(out.dir, "/survreg_run", Sys.Date(), ".txt", sep = ""))
survplot()

survreg_plate <- survreg(Surv(end, status) ~ trt + region + trt:region + frailty(plate%in%run) + frailty(run), data = df1)
out <- capture.output(survreg_plate)
cat("RegressionOutput", out, file = paste(out.dir, "/survreg_plate", Sys.Date(), ".txt", sep = ""))
...
out <- capture.output(survreg_plate_line)
cat("RegressionOutput", out, file = paste(out.dir, "/survreg_", Sys.Date(), ".txt", sep = ""))
