...
jpeg('test_region_1_20170504.jpg')
...
test.landrace.1 <- survfit(Surv(end, status) ~ landrace.name, data = df1, type = "kaplan-meier", conf.type = "log-log")
jpeg('test_landrace_1_20170504.jpg')
plot(test.landrace.1, col = rainbow(18), lwd = 1.4, lty = c(1:6), xlim = c(0,550), main = "Germination by landrace")
legend(5, .8, levels(df1$landrace.name), 
       col =  rainbow(18), lwd = 1.75, lty=c(1:6), bty = "n", cex = 1)
title(xlab = "time (hours)", line = 2, ylab = "probability of not germinating")
dev.off()

test.shelf.1 <- survfit(Surv(end, status) ~ shelf, data = df1, type = "kaplan-meier")
jpeg('test_shelf_1_20170504.jpg')
plot(test.shelf.1, col = rainbow(9), lwd = 1.4, xlim = c(0,550), main = "Germination by Shelf")
title(xlab = "time (hours)", line = 2, ylab = "probability of not germinating")
dev.off()

test.cultivation.1 <- survfit(Surv(end, status) ~ cultivation, data = df1, type = "kaplan-meier")
jpeg('test_cultivation_1_20170504.jpg')
plot(test.cultivation.1, col = rainbow(5), lty = 1:5, lwd = 1.75, xlim = c(0,550), main= "Germination by cultivation")
...
#log rank test
survdiff_20_logrank <- survdiff(Surv(end, status)~region, data = df1, 
          subset = {trt==20  & region!= "wcoast"}, rho = 0) #log rank test
