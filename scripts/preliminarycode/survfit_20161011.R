...
test.type.1 <- survfit(Surv(end, status) ~ type, data = df1, type = "kaplan-meier")
plot(test.type.1, col = c("blue", "red", "yellow"), lwd = 1.4, xlim = c(0,550))
...
plot(test.region.1, mark.time = F, fun = mlogmlog, log = "x", xlab = "t (hours)", ylab = "-log(-log(S(t)))", 
     col = c("black", "red", "grey","green", "cyan", "blue")) #looks good!
...
cox.0 #significant p =0.00022
cox.1 <- coxph(Surv(end,status) ~ trt10, data = df1)
cox.1 #significant p = 5.9e-08
cox.2 <- coxph(Surv(end,status) ~ trt15, data = df1)
cox.2 #NS
cox.3 <- coxph(Surv(end,status) ~ trt20, data = df1)
cox.3 #significant p = 5.1e-15
cox.4 <- coxph(Surv(end,status) ~ cv, data = df1)
cox.4 #significant p = 2.1e-15
cox.5 <- coxph(Surv(end,status) ~ sierramadre, data = df1)
cox.5 #NS
cox.6 <- coxph(Surv(end,status) ~ ecoast, data = df1)
cox.6 #significant p = 0.031
cox.7 <- coxph(Surv(end, status) ~ wcoast, data = df1)
cox.7 #significant p = <2e-16
cox.8 <- coxph(Surv(end, status) ~ yucatan, data = df1)
cox.8 #significant p = .018
cox.9 <- coxph(Surv(end, status) ~ johnny, data = df1)
cox.9 #significant p = 2e-06

#add covariates in order of significance 
cox.10 <- coxph(Surv(end,status) ~ wcoast + trt20 +frailty(run) + frailty(type), data = df1)
cox.10 #all significant
cox.11 <- coxph(Surv(end,status) ~ wcoast + trt20 + cv +frailty(run) + frailty(type), data = df1)
cox.11 #all significant
cox.12 <- coxph(Surv(end,status) ~ wcoast + trt20 +cv +trt10 + frailty(run) + frailty(type), data = df1)
cox.12 #all significant
cox.13 <- coxph(Surv(end,status) ~ wcoast + trt20 +cv +trt10 + johnny + frailty(run) + frailty(type), data = df1)
cox.13 # NS
cox.14 <- coxph(Surv(end,status) ~  wcoast + trt20 +cv +trt10 + trt0 + frailty(run) + frailty(type), data = df1)
cox.14 # all significant
cox.15 <- coxph(Surv(end,status) ~ wcoast + trt20 +cv +trt10 + trt0 + yucatan+ frailty(run) + frailty(type), data = df1)
cox.15 #all significant
cox.16 <- coxph(Surv(end,status) ~ wcoast + trt20 +cv +trt10 + trt0 + yucatan+ ecoast+ frailty(run) + frailty(type), data = df1)
cox.16 #MS

#testing interactions
cox.20 <- coxph(Surv(end,status) ~  wcoast + trt20 +cv +trt10 + trt0 + yucatan+ frailty(run) + frailty(type) + wcoast:trt20, data = df1)
cox.20 #NS
cox.21 <- coxph(Surv(end,status) ~ wcoast + trt20 +cv +trt10 + trt0 + yucatan+ frailty(run) + frailty(type) + wcoast:trt10, data = df1)
cox.21 #NS
cox.22 <- coxph(Surv(end,status) ~ wcoast + trt20 +cv +trt10 + trt0 + yucatan+ frailty(run) + frailty(type) + wcoast:trt0, data = df1)
cox.22 #NS
cox.23 <- coxph(Surv(end,status) ~ wcoast + trt20 +cv +trt10 + trt0 + yucatan+ frailty(run) + frailty(type) + cv:trt20, data = df1)
cox.23
cox.24 <- coxph(Surv(end,status) ~ wcoast + trt20 +cv +trt10 + trt0 + yucatan+ frailty(run) + frailty(type) + cv:trt20 + cv:trt10, data = df1)
cox.24 #NS
cox.25 <- coxph(Surv(end,status) ~ wcoast + trt20 +cv +trt10 + trt0 + yucatan+ frailty(run) + frailty(type) + cv:trt20 + cv:trt0, data = df1)
cox.25 #NS
cox.26 <- coxph(Surv(end,status) ~ wcoast + trt20 +cv +trt10 + trt0 + yucatan+ frailty(run) + frailty(type) + cv:trt20 + yucatan:trt20, data = df1)
cox.26 #NS
cox.27 <- coxph(Surv(end,status) ~ wcoast + trt20 +cv +trt10 + trt0 + yucatan+ frailty(run) + frailty(type) + cv:trt20 + yucatan:trt10, data = df1)
cox.27 #NS
cox.28 <- coxph(Surv(end,status) ~ wcoast + trt20 +cv +trt10 + trt0 + yucatan+ frailty(run) + frailty(type) + cv:trt20 + yucatan:trt0, data = df1)
cox.28 #NS

print(summary(cox.23))
